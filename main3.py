            ...
            if pelaajaElossa==True and peliLapaisty==False:
              ...
              #  print("Choose your next action:\n")
                ...
                for action in availableActions:
                   print("   -"+action)
                print("")
                print("Hotkeys: 'Move [n,s,e,w]', Examine [item name], Pick [item name], Drop [item name], Use [item name], Fight, Inventory, Talk")
                print("")
...
def pueItem(nimi):  #pukee varusteen tai aseen pelaajalle
    ...
    sql2 = "SELECT charisma FROM player"
    cur.execute(sql)
    result = cur.fetchall()
    ...
    while(vihutElossa == True and pelaajaElossa==True):
        ...
        if tee=="fight":# or "f":
            ...
            if huoneId == 9:
                global vihuElossa5
                if  vihuElossa5 == True:
                    vihuIske(10, vihuId)  #BOSSEN DAMAGE TÄNNE
                else:
                    vihutElossa = False
                    print("The enemy is dead")
